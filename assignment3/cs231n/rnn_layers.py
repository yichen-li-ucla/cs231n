...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dx[:,t,:],dh_t,dWx_t,dWh_t,db_t=rnn_step_backward(dh[:,t,:]+dh_t,cache.pop())
      dWx+=dWx_t
      dWh+=dWh_t
      db+=db_t
    dh0=dh_t
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # L=f(h,c), h=g(c), dL = pf/ph dh + pf/pc dc = (pf/ph * pg/pc + pf/pc) dc
    dnext_c+=dnext_h*o*(1-np.tanh(next_c)**2) # y=tanh(x),dy/dx=(1-y^2)
    do=dnext_h*np.tanh(next_c)
    di=dnext_c*g
    df=dnext_c*prev_c
    dg=dnext_c*i
    dprev_c=dnext_c*f
    da_o=do*o*(1-o) # y=sigmoid(x),dy/dx=y(1-y)
    da_i=di*i*(1-i)
    da_f=df*f*(1-f)
    da_g=dg*(1-g**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_h=h0
    for t in range(T):
      next_h,c,cach=lstm_step_forward(x[:,t,:],prev_h,c,Wx,Wh,b)
      h[:,t,:]=next_h
      cache.append(cach)
      prev_h=next_h
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dx[:,t,:],dprev_h,dprev_c,dWx_t,dWh_t,db_t=lstm_step_backward(dh[:,t,:]+dprev_h,dprev_c,cache.pop())
      dWx+=dWx_t
      dWh+=dWh_t
      db+=db_t
    dh0=dprev_h
